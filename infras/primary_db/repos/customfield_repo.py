...
class CustomFieldsRepo:
    ...
    async def delete_field(self,data:DeleteCustomFieldDbSchema) -> bool:
        stmt = delete(PurchaseCustomFields).where(PurchaseCustomFields.id == (data.id if hasattr(data, "id") and data.id else getattr(data, "field_id", None)), PurchaseCustomFields.shop_id == data.shop_id)
    ...
    async def get_fields_by_shop_id(self, data:GetFieldByShopIdSchema) -> List[dict]:
        ...
        if field_ids:
            val_stmt = select(PurchaseCustomFieldsValues.field_id).where(
                PurchaseCustomFieldsValues.field_id.in_(field_ids),
                PurchaseCustomFieldsValues.shop_id == data.shop_id,
                PurchaseCustomFieldsValues.value.isnot(None),
                PurchaseCustomFieldsValues.value != ""
            ).distinct()
    ...
    async def get_values_by_field_id(self, field_id: str, shop_id: str) -> List[dict]:
        stmt = select(PurchaseCustomFieldsValues).where(
            PurchaseCustomFieldsValues.field_id == field_id,
            PurchaseCustomFieldsValues.shop_id == shop_id
        )
        res = (await self.session.execute(stmt)).scalars().all()
        return [{c.name: getattr(row, c.name) for c in row.__table__.columns} for row in res]
