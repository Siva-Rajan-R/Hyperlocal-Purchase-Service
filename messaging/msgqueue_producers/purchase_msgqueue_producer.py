...
async def verify_and_update(purchase_data: dict, headers: dict, payload: dict, rabbitmq_connection: Any, user_infos: Optional[dict] = None):
    ...
    for item in purchase_data.get('items', []):
        ...
        body.append({
            ...
            'online_sell_price': pricing_infos.get('sell_price', 0),
            ...
            try:
                ...
                async with AsyncInventoryLocalSession() as session:
                    ...
                    purchase_read_model = PurchaseReadModel(
                        ...
                        total_quantity=float(item_infos.get("total_pur_stocks", 0)),
                        item_infos=item_infos,
                        payment_infos=payment_infos,
                ...
                try:
                    ...
                    asyncio.create_task(emit_notification(
                        ...
                        message=f"Purchase order with invoice '{invoice_no}' has been successfully processed.",
                        type="info",
                ...
                try:
                    ...
                    asyncio.create_task(emit_notification(
                        ...
                        message=f"Failed to process purchase order '{purchase_data.get('invoice_no')}': {str(e)}",
                        type="error",
                        user_id=executing_user_id or purchase_data.get("shop_id")
                    ))
                except Exception as notification_error:
                    ic(f"Notification error: {notification_error}")
                raise e
